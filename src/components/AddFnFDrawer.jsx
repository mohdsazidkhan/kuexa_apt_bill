...
const dummyMemberships = [
  ...
  { id: 3, name: 'Prive Package', type: 'Package', price: 2000, end: '14-11-2025', added: false },
  { id: 4, name: 'Birthday Giftcard', type: 'Giftcard', price: 500, end: '20-12-2024', added: false },
  ...
  { id: 7, name: 'Bridal Package', type: 'Package', price: 15000, end: '28-11-2025', added: false },
  ...
  return (
    <>
      ...
      >
          ...
          <table className="w-full text-left text-sm text-gray-700">
            <thead className="sticky top-0 bg-white">
              <tr className="border-b border-gray-200 font-bold text-[#2c4c6b]">
                <th className="px-2 py-1">Offer Name</th>
                <th className="px-2 py-1">Offer Type</th>
                <th className="px-2 py-1">Price</th>
                <th className="px-2 py-1">Benefit End Date</th>
            ...
            <tbody className="divide-y divide-gray-300">
              {dummyMemberships.map((offer) => (
                <tr key={offer.id} className="hover:bg-gray-50/50">
                  <td className="px-2 py-1 font-semibold text-gray-600">{offer.name}</td>
                  <td className="px-2 py-1">
                    <span className={`rounded-md px-2 py-1 text-xs font-semibold ${
                      offer.type === 'Membership' ? 'bg-purple-100 text-purple-700' :
                      offer.type === 'Package' ? 'bg-cyan-100 text-cyan-700' :
                      offer.type === 'Giftcard' || offer.type === 'Gift Card' ? 'bg-green-100 text-green-700' :
                      'bg-gray-100 text-gray-700'
                    }`}>
                      {offer.type}
                    </span>
                  </td>
                  <td className="px-2 py-1">{currency(offer.price)}</td>
                  <td className="px-2 py-1">{offer.end}</td>
                  <td className="px-2 py-1 text-center">
                    <button
                      onClick={() => toggleOffer(offer.id)}
                      className={`flex h-6 w-6 items-center justify-center mx-auto rounded border ${selectedOffers.includes(offer.id)
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 border-t border-gray-200 bg-white px-2 py-1">
          <button
            onClick={onClose}
